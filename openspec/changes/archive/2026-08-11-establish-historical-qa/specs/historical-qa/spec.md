...
The `0004_historical_qa` migration SHALL create only restrictive `app.historical_qa` records under active Shenzhen scope with immutable `(source_url, content_sha256)` uniqueness, RLS, no browser policy/grant, and no `auth.users` mutation.
...
The backend SHALL expose `GET /api/v1/qa` and `GET /api/v1/qa/{qa_id}` behind current identity. List SHALL use projected one-statement window-total pagination for non-empty pages; both endpoints SHALL scope to Shenzhen, preserve shared errors/request ID behavior, and set `Cache-Control: no-store`.
