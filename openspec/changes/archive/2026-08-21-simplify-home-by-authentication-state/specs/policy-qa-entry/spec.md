...
The frontend SHALL provide a responsive policy-Q&A workbench as the primary visual entry only for authenticated individual/enterprise homepage and service-entry views, using the established visual baseline and independently authored visual treatment. The signed-out homepage SHALL instead show its real personal and enterprise service entrances without mounting a Q&A textarea, chat result or question-submission affordance. The workbench SHALL present empty, loading, explicit placeholder, source-empty and error states without client-side answer fallback, history, import or model controls.
