...
- **THEN** the system returns the established safe authorization or validation error and creates no consultation, message or status event
...
- **THEN** the system returns a stable conflict or authorization error, preserves the previously committed state, and creates no extra message or history event
