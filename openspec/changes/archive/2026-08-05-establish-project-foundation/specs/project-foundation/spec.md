...
为后续政策、问答、咨询和智能能力提供一个可以在新机器上重复启动、检查和继续扩展的最小工程底座。
...
The service SHALL return application errors in one JSON shape containing a stable error code, a human-readable message, optional details, and the request ID. Every response SHALL include the request ID in the `X-Request-ID` header, preserving a valid caller-supplied ID or generating one when absent.
...
The repository SHALL provide documented commands to install dependencies, start the frontend and backend separately, run checks, apply database migrations, and execute a smoke test. The frontend SHALL provide a working root route and a service-status route, and SHALL display a recoverable connection error when the API is unavailable.
